...
The graphical profile for NVI is published on the NVI intranet. The different 
colour palettes in the NVI graphical profile is displayed below. The palettes 
are collected in the R-package NVIpretty. The palette colours are named.
...
The NVI profile has 15 different colours for use in graphs. These are collected 
in the palette: "palette_graph". The palette colours are unnamed, but are based 
on the NVI profile colours. The colours should be used in the order they are in 
the palette. 
...
## The NVI colours for graphs used in the fish health report and animal health report

The NVI profile has 12 different colours for use in graphs. These are collected 
in the palette: "palette_graph". The palette colours are unnamed. The colours 
should be used in the order they are in the palette. For line graphs, the colours 
<span style="color: #F2C512;">#F2C512</span> and 
<span style="color: #C0ECD3;">#C0ECD3</span> should not be used.

```{r fig.width = 5, fig.height = 4, echo = FALSE}
# names(palette_graph) <- c("Jord", "Luft", "Vann", "Svart", "Lys jord",
#                           "Mørk luft", "Lys vann", "Mat", "Lys mat", "Gul",
#                           "Lys gul", "Lilla", "Lys lilla", "Orange", "Lys orange")
display_palett(palette_graph_report, reverse_colours = TRUE)
```

## The NVI colours for gradients

There are six different colour gradients based on the NVI profile colours. Within 
each gradient, there are six different colours. These are intended for choropleth 
maps and can also be used in graphs. The palette colours are unnamed.
